...
from scripts.policy_objects.delete_policy_objects import delete_policy_objects
import threading

class DeletePolicyObjectsWindow(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Delete Policy Objects")
    ...
    def create_widgets(self):
            # Title label
            ttk.Label(self, text="Delete Policy Objects from CSV", font=("Segoe UI", 14)).pack(pady=10)
    ...
    def run_deletion(self):
        try:
            ...
            result = delete_policy_objects(csv_file=selected_file, log_callback=self.log_to_gui, debug=True)
            
            if result:
                messagebox.showinfo("Summary", result["summary"])
            else:
                messagebox.showerror("Error", "Object deletion failed. Check the log.")
        finally:
