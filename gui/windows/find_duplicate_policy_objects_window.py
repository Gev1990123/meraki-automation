...
from scripts.policy_objects.find_duplicate_policy_objects import find_duplicate_policy_objects
import threading

class FindDuplicatePolicyObjectsWindow(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Find Duplicate Policy Objects")
    ...
    def create_widgets(self):
            # Title label
            ttk.Label(self, text="Export Duplicate Objects to CSV", font=("Segoe UI", 14)).pack(pady=10)
    ...
    def run_export(self):
        try:
            ...
            result = find_duplicate_policy_objects(csv_file=selected_file, log_callback=self.log_to_gui, debug=True)
