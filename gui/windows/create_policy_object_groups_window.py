...
from scripts.policy_objects.create_policy_object_groups import create_policy_object_groups
import threading

class CreatePolicyObjectGroupsWindow(tk.Toplevel):
    def __init__(self, master=None):
        super().__init__(master)
        self.title("Create Policy Object Group")
    ...
    def create_widgets(self):
            # Title label
            ttk.Label(self, text="Create Policy Object Groups from CSV", font=("Segoe UI", 14)).pack(pady=10)
            ...
            # Start button
            self.start_button = ttk.Button(self, text="Start Creation", command=self.run_creation_threaded)
    ...
    def run_creation(self):
        try:
            ...
            result = create_policy_object_groups(csv_file=selected_file, log_callback=self.log_to_gui, debug=True)
